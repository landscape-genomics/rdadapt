...
res_RDA_proj_current <- adaptive_index(RDA = RDA_outliers, K = 2, env = xx.ras.current, env_mask = NULL, method = "loadings", scale_env = NULL, center_env = NULL)
```
...
res_RDA_proj <- genomic_offset(RDA = RDA_outliers, K = 2, env_pres = xx.ras.current, env_fut = xx.ras.future, env_mask = NULL, method = "loadings", scale_env = NULL, center_env = NULL)
...
6.  Finding covaryaing sets of individuals or loci on the RDA space

    </h2>

    <br>

<h4>6.1. Identifying "modules" of covarying loci</h4>
...
<h4>6.2. Finding groups of individuals with the same adaptive background</h4>

Blablabla

```{r, message=FALSE, results='hide', eval=TRUE, fig.align='center'}
## Randomly select only 20 populations
samp <- which(pop %in% sample(1:100, 20, replace = FALSE))

coord.samp <- coord[samp,]

points(coord.samp, col = "red", add = T)}
```
