...
xx.ras.current <- rast(data.frame(x = rep(1:10, time = 10), y = rep(1:10, each = 10), aggregate(xx.current, by = list(pop), mean)[,-1]), type="xyz", crs = crs("epsg:4326"))
plot(xx.ras.current)

xx.ras.future <- rast(data.frame(x = rep(1:10, time = 10),y = rep(1:10, each = 10), aggregate(xx.future, by = list(pop), mean)[,-1]), type="xyz", crs = crs("epsg:4326"))
plot(xx.ras.future)
```
