...
We show how to run the package using data simulated with Slim3 as described in [Gain et al. 2023](https://doi.org/10.1093/molbev/msad140). `maladapt` dataset contains genotypes (*Y*) for 2036 simulated individuals on a 12\*12 grid constrained by two causal environmental variables (*Var1* and *Var2*). Fitness (*fitness*) was measured for all individuals before and after a brutal change in environment.
...
```{r, message=FALSE, warning=FALSE, results='hide', eval=TRUE, fig.align='center', fig.height = 8, fig.width= 8}
## Population labels
pop <- maladapt$pop
...
<h4>1.2 Subsample populations</h4>
...
locus_scores <- as.data.frame(scores(rdadapt_env, choices=c(1:2), display="species", scaling="none"))
...
TAB_var <- as.data.frame(scores(rdadapt_env, choices=c(1,2), display="bp")) 
...
```{r, message=FALSE, warning=FALSE, results='hide', eval=TRUE, fig.align='center', fig.width=10, echo=FALSE}
...
```{r, message=FALSE, warning=FALSE, results='hide', eval=TRUE, fig.align='center', fig.width=10, fig.height = 4}
...
ggplot() +
  ...
  theme(panel.grid = element_blank()
        ...
        , strip.text = element_text(size = 11))
```
```{r, message=FALSE, warning=FALSE, results='hide', eval=TRUE, fig.align='center', fig.height = 4, fig.width= 10}
...
```{r, message=FALSE, results='hide', eval=TRUE, fig.align='center', fig.align='center', fig.height = 4, fig.width= 5}
...
plot(coord.samp, cex = 0.2, xlim = c(0, 12), ylim = c(0, 12), col = groups$samples$cluster)
